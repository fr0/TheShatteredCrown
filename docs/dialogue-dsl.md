...
| `smith_repair()` | opens the forge's repair window: every damaged weapon and piece of armor the party carries on this map, priced off market value and village standing. `smith_repair(free)` for a smith who is not taking coin (Garrick, once Bry is home). Each mending permanently lowers that piece's HP ceiling by 10%, floor 60% |
| `stable_animals()` | opens the town stable's line of pack animals: every tameable pack beast the load order has, priced off its own worth with the stabler's markup and the village's opinion of you. Bought beasts arrive tame, obedient and in the player's faction |
