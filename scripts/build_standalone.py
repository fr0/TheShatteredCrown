...
Run:  py scripts/build_standalone.py [--set-version x.y.z]

The standalone versions INDEPENDENTLY of The Shattered Crown: its number
lives in docs/standalone/version.txt (created at 1.0.0), is bumped with
--set-version, and is stamped into the generated csproj and About.xml.
"""
import argparse
import os
import re
...
VERSION_FILE = os.path.join(ASSETS, "version.txt")

parser = argparse.ArgumentParser()
parser.add_argument("--set-version", dest="set_version", metavar="x.y.z",
                    help="update the standalone's own version before building")
args = parser.parse_args()

if args.set_version:
    if not re.fullmatch(r"\d+\.\d+\.\d+", args.set_version):
        sys.exit(f"version must look like 1.2.3 (got '{args.set_version}')")
    os.makedirs(ASSETS, exist_ok=True)
    open(VERSION_FILE, "w", encoding="ascii", newline="").write(args.set_version)
    print(f"standalone version set to {args.set_version}")

VERSION = (open(VERSION_FILE, encoding="ascii").read().strip()
           if os.path.exists(VERSION_FILE) else "1.0.0")
